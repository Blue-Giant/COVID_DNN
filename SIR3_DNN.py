...
def print_and_log2train(i_epoch, run_time, tmp_lr, temp_penalty_nt, penalty_wb2sir, penalty_wb2paras,
    ...
    print('penalty weights and biases for paras: %f' % penalty_wb2paras)
    ...
    DNN_tools.log_string('penalty weights and biases for paras: %f' % penalty_wb2paras, log_out)
...
def solve_SIR2COVID(R):
    ...
    if R['model'] == 'DNN_Cos_C_Sin_Base':
        outDimSIR = 3
        out_dim2paras = 2
        Weight2SIR, Bias2SIR = DNN_base.initialize_NN_random_normal2_CS(input_dim, outDimSIR, hidden_layers, flag2SIR)
        Weight2paras, Bias2paras = DNN_base.initialize_NN_random_normal2_CS(input_dim, out_dim2paras, hidden_layers, flag2paras)
    else:
        outDimSIR = 3
        out_dim2paras = 2
        Weight2SIR, Bias2SIR = DNN_base.initialize_NN_random_normal2(input_dim, outDimSIR, hidden_layers, flag2SIR)
        Weight2paras, Bias2paras = DNN_base.initialize_NN_random_normal2(input_dim, out_dim2paras, hidden_layers, flag2paras)
    ...
    with tf.device('/gpu:%s' % (R['gpuNo'])):
        with tf.variable_scope('vscope', reuse=tf.AUTO_REUSE):
            ...
            if 'PDE_DNN' == str.upper(R['model']):
                SIR_NN_temp = DNN_base.PDE_DNN(T_it, Weight2SIR, Bias2SIR, hidden_layers, activate_name=act_func)
                in_paras = DNN_base.PDE_DNN(T_it, Weight2paras, Bias2paras, hidden_layers, activate_name=act_func)
            elif 'PDE_DNN_Fourier' == R['model']:
                SIR_NN_temp = DNN_base.DNN_Fourier_Base(T_it, Weight2SIR, Bias2SIR, hidden_layers, activate_name=act_func)
                in_paras = DNN_base.DNN_Fourier_Base(T_it, Weight2paras, Bias2paras, hidden_layers, activate_name=act_func)
            elif 'DNN_Cos_C_Sin_Base' == R['model']:
                freq = [1]
                SIR_NN_temp = DNN_base.DNN_Cos_C_Sin_Base(T_it, Weight2SIR, Bias2SIR, hidden_layers, freq_frag=freq, activate_name=act_func)
                in_paras = DNN_base.DNN_Cos_C_Sin_Base(T_it, Weight2paras, Bias2paras, hidden_layers, freq_frag=freq, activate_name=act_func)
            elif 'PDE_DNN_SCALEOUT' == str.upper(R['model']):
                freq = np.concatenate(([1], np.arange(1, 20)), axis=0)
                SIR_NN_temp = DNN_base.PDE_DNN_scaleOut(T_it, Weight2SIR, Bias2SIR, hidden_layers, freq, activate_name=act_func)
                in_paras = DNN_base.PDE_DNN_scaleOut(T_it, Weight2paras, Bias2paras, hidden_layers, freq, activate_name=act_func)

            S_NN_temp = tf.gather(SIR_NN_temp, [0], axis=-1)
            I_NN_temp = tf.gather(SIR_NN_temp, [1], axis=-1)
            R_NN_temp = tf.gather(SIR_NN_temp, [2], axis=-1)
            in_beta = tf.gather(in_paras, [0], axis=-1)
            in_gamma = tf.gather(in_paras, [1], axis=-1)
            ...
            if R['regular_weight_model'] == 'L1':
                ...
                regular_WB2PARAS = DNN_base.regular_weights_biases_L1(Weight2SIR, Bias2SIR)
            elif R['regular_weight_model'] == 'L2':
                regular_WB2SIR = DNN_base.regular_weights_biases_L2(Weight2SIR, Bias2SIR)
                regular_WB2PARAS = DNN_base.regular_weights_biases_L2(Weight2SIR, Bias2SIR)
            else:
                regular_WB2SIR = tf.constant(0.0)
                regular_WB2PARAS = tf.constant(0.0)

            PWB2SIR = wb_penalty*regular_WB2SIR
            PWB2PARAS = wb_penalty * regular_WB2PARAS
    ...
    with tf.Session(config=config) as sess:
        ...
        for i_epoch in range(R['max_epoch'] + 1):
            ...
            _, loss_s, loss_i, loss_r, loss_n, pwb2sir, pwb2para = sess.run(
                [train_Loss, Loss2S, Loss2I, Loss2R, Loss2All, PWB2SIR, PWB2PARAS],
            ...
            if i_epoch % 1000 == 0:
                ...
                print_and_log2train(i_epoch, time.time() - t0, tmp_lr, temp_penalty_pt, pwb2sir, pwb2para, loss_s,
...
if __name__ == "__main__":
    ...
    if R['model'] == 'DNN_Cos_C_Sin_Base':
        ...
        R['hidden_layers'] = (50, 100, 50, 50, 30)  # 1*50+50*50+50*30+30*30+30*20+20*1 = 5570
    else:
        ...
        R['hidden_layers'] = (100, 100, 50, 50, 30)  # 1*50+50*50+50*30+30*30+30*20+20*1 = 5570
